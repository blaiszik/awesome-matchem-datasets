...
Added 17 new datasets covering releases since the October 2025 cutoff: universal/foundation-model training data (catalysis, polymers, molecules), the DTU/CAMD family of 2D-materials databases, experimental NMR and protein–ligand binding data, actinide/lanthanide separation chemistry, reaction templates, and a new generation of chemistry/materials LLM reasoning benchmarks. Each entry below carries a **🔗 Verify** line with a direct source link and the key figures so additions can be checked quickly. (A few notable Sept–Oct 2025 datasets missed in the prior update are included; the five DTU/CAMD 2D-materials databases — BiDB, HetDB, QPOD, 2D Alloy, X2DB — were contributed by the community via PR.)
